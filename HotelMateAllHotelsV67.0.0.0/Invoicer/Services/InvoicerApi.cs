...
namespace Invoicer.Services
{
    public class InvoicerApi : IInvoicerApi
    {
        ...
        public InvoicerApi(
            ...
            string currency = "NGN", string title = "RECEIPT"
            )
        {
            Invoice = new Invoice();
            Invoice.Title = title;
        ...
        public void Save(string filename, string password = null)
        {
            if (filename == null || !System.IO.Path.HasExtension(filename))
                filename = @"C:\NewProjects\Juju\" + System.IO.Path.ChangeExtension(Invoice.Reference, "pdf");

           new PdfInvoice(Invoice).Save(filename, password);
        }

        public string Save(string filename, int checkout = 0)
        {
            if (filename == null || !System.IO.Path.HasExtension(filename))
                filename = filename + System.IO.Path.ChangeExtension(Invoice.Reference, "pdf");

            new PdfInvoice(Invoice).Save(filename, "", checkout);
            return filename;
        }
    }
}
