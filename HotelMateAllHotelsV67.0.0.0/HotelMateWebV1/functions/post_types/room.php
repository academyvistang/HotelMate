...
function ci_update_cpt_room_meta($post_id)
{
	...
	if (isset($_POST['post_type']) && $_POST['post_type'] == "room")
	{
		...
		update_post_meta($post_id, "ci_cpt_room_amenities_title", (isset($_POST["ci_cpt_room_amenities_title"]) ? $_POST["ci_cpt_room_amenities_title"] : '') );
...
function ci_add_cpt_room_meta_box()
{
	...
	<p><?php _e('When the slider is not enabled, or you don\'t have any images assigned to the room, nothing will be displayed. The Featured Image will be displayed nonetheless.', 'ci_theme'); ?></p>
	...
	<p>
		...
		<label for="ci_cpt_room_slider_not"><?php _e('Don\'t add a slider to this room.', 'ci_theme'); ?></label>
	</p>

	<h2><?php _e('Amenities', 'ci_theme'); ?></h2>
	<p><?php _e('Provide the amenities of the room. Select <b>Add Field</b> as many times as you want to create a list of amenities. You can delete one by clicking on its <b>Remove me</b> link next to it. You may also click and drag the fields to re-arrange them. The amenities can be shown with the CI Amenities widget' , 'ci_theme'); ?></p>
	<p>
		<?php 
	...
	<p>
		<label for="ci_cpt_room_from"><?php _e('From price', 'ci_theme'); ?></label><br>
		<input id="ci_cpt_room_from" name="ci_cpt_room_from" type="text" value="<?php echo esc_attr($ci_room_from); ?>" class="code" style="width:230px" />
	</p>
