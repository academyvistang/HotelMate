...
{
	global $post;
	$slider = get_post_meta($post->ID, 'ci_cpt_gallery_slider', true);
	?>
	...
	<p><?php _e('You may disable the gallery slider that appears on the gallery\'s single page. If you do, a simple grid of images will be displayed instead.', 'ci_theme'); ?></p>
	<p>
		<input type="checkbox" id="ci_cpt_gallery_slider" name="ci_cpt_gallery_slider" value="disabled" <?php checked($slider, 'disabled'); ?> /> 
		<label for="ci_cpt_gallery_slider"><?php _e('Don\'t add a slider to this post (show gallery).', 'ci_theme'); ?></label>
	</p>
