...
namespace HotelMateWebV1.Controllers
{
    ...
    public class ReportController : PdfViewController
    {
        ...
        public ActionResult BalanceSheetBar(DateTime? startDate, DateTime? endDate)
        {
            ...
            var balanceSheetModel = accountsPaidin.Select(x => new BalanceSheetModel { TransactionDate = x.DateSold.Value, AmountPaidIn = x.TotalPrice, AmountPaidOut = decimal.Zero, Cashier = x.TransactionId, PaymentMentMethod = x.PaymentMethod, PaymentTypeId = x.PaymentTypeId }).ToList();
