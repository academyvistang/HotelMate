<?php
/**
 * Plugin installation and activation for Leboston themes.
 *
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	/**
 	 ...
 	 * The plugins can be either pre-packaged, downloaded from the Leboston
	...
	class TGM_Plugin_Activation {
		...
		/**
		 * Initialise the interactions between this class and Leboston.
		 *
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				/** Set plugin source to Leboston API link if available */
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				/** The plugin is from the Leboston repository */
				else {
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				/** Loop through each plugin to install and try to grab information from Leboston API, if not create 'tgmpa-empty' scalar */
 ...
 * we load it here.
 *
 * We check to make sure no action or activation keys are set so that Leboston
